...
void seam(Mat im, Mat oldSeamsHor, Mat oldSeamsVer, int numOldSeamsHor, int numOldSeamsVer, int numNewSeamsHor, int numNewSeamsVer){
	...
	for(int i = 0; i < im.rows; ++i){
		int c=0;
		for(int j = 0; j < im.cols; ++j){
			if(oldSeamsVer.at<uchar>(i,j)==255){
				c++;
			}
		}
		if(c!=numOldSeamsVer + numNewSeamsVer){
			cout<<i<<endl;
			cout << "not vertical " << c << endl;
			//exit(0);
		}
	}
}

Mat showSeams(Mat im, vvi indMat, Mat seams, bool useIndMat){
		...
		for(int j = 0; j < seams.cols; ++j){
			if(seams.at<uchar>(i,j)==255){
				if(!useIndMat)
					...
					displayIm.at<Vec3b>(i,indMat[i][j]) = Vec3b(0,0,255);
			}
		}
	}
